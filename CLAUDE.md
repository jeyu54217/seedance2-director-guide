...
**壓縮比**：每篇 Q 目標 60–80 行。能用一句講完，不用三句。段落短，邏輯密。

**禁止項目**：
- 華麗比喻（「攪成一鍋粥」「會議室裡的翻譯員」「就像交響樂團指揮」）——直接講機制
- 過渡廢話（「這是融合的核心」「機制講完了，回到導演最關心的問題」「先想一個問題」）——直接進入下一點
- 重複敘述——表格已寫的不要在段落重講、前一步已提的不要在後一步再展開
- 長案例——一個具體名詞示範足以（「金髮」），不要鋪陳三行場景（「角色從椅子上站起來...」）

**保留項目**：
- 技術公式、表格、論文引用（一個 cite block 放文末即可，不要灑在中間）
- 跨 Q 連結
- 導演要點每項 1–2 句

**範例**：WHY 段不要寫「文字走 Text Encoder 產出 text embedding，圖片走 VAE Encoder 產出 latent，三條路各自算完，最後硬接在一起。結果是：文字 token 從頭到尾沒見過圖片 token。文字說『金髮雙馬尾』，圖片 token 不知道有這個約束...」——壓成：「各模態分開處理，不同模態的 token 從未在同一個計算空間中互相對齊。文字說『金髮』，圖片 token 不知道。」
